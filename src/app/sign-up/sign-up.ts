...
export class SignUp implements AfterViewInit, OnDestroy {
  ...
  verifyEmail() {
    ...
    this.otpSentMessage = '';
    this.http.post<any>('http://54.198.56.106:8081/api/users/request-otp', { email }).subscribe({
      next: (res) => {
  ...
  validateOtp() {
    ...
    this.http.post('http://54.198.56.106:8081/api/users/verify-otp', { email, otp }).subscribe({
      next: () => {
  ...
  onSubmit() {
    ...
    this.http.post('http://54.198.56.106:8081/api/users/register', formValue).subscribe({
      next: () => {
